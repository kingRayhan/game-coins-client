  ...
  <div>
    <div class="level">
      <div class="level-left">
        <h2 class="text-3xl uppercase">Done Orders</h2>
      </div>
      <div class="level-right">
        <nuxt-link :to="{ name: 'dashboard-orders' }" class="button is-primary">
  ...
  methods: {
    async loadData() {
      ...
      const items = await this.$axios.$get(
        `orders?status=DONE&limit=${this.perPage}&page=${this.currentPage}`
      );
      this.loading = false;
